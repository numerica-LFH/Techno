# Séance 2 — Qu'est-ce qui change dans un objet qui évolue ?

!!! info "Séance 2 sur 3 · 55 minutes"

    Fonction technique, principe technique, design.

    [Fiche élève à imprimer (PDF)](fiches/4e-seq1-seance2-eleve.pdf)

## AVANT — j'entre dans la question

#### Ce qu'on cherche

En 1946, le premier ordinateur pesait 30 tonnes et occupait 167 m². Celui qui tient dans mon sac fait 1,5 kg et calcule un million de fois plus vite.

**Quand un objet nouveau apparaît, est-ce que tout est changé en lui ?**

J'écris trois choses que je crois savoir. Je n'ai pas besoin d'avoir raison : je reviendrai sur ces lignes à la fin de l'heure.

| N°  | Mes hypothèses de départ | Vérifié en fin de séance (✔ / ✘) |
|-----|--------------------------|----------------------------------|
| 1   |                          |                                  |
| 2   |                          |                                  |
| 3   |                          |                                  |

#### Vocabulaire à repérer

| Mot                    | Ce que j'en comprends, avec mes mots |
|------------------------|--------------------------------------|
| **fonction technique** |                                      |
| **principe technique** |                                      |
| **design**             |                                      |

## PENDANT — je recherche

### Activité 1 · L'ENIAC, premier ordinateur

Pendant la Guerre froide, période de tension entre les États-Unis et l'URSS, l'armée américaine a eu besoin de calculer la trajectoire des missiles et d'améliorer leur précision. Pour cela, il a fallu construire une machine capable d'effectuer de nombreuses opérations arithmétiques. L'ENIAC est la première machine programmée et électronique, ce qui fait d'elle le premier ordinateur. Avant l'ENIAC, les machines à calculer étaient mécanisées.

*ENIAC, 1946. Poids : 30 tonnes. Taille : 167 m². Consommation : 150 kilowatts.*

a\. Quels sont les inconvénients de l'ENIAC ?

b\. Je relève les deux passages du texte qui font référence au contexte historique et politique.

c\. Je relève les deux passages qui font référence au contexte technique et scientifique.

d\. Quel composant inventé en 1971 a permis de réduire la taille des ordinateurs au point d'en faire des ordinateurs personnels ? .........

### Activité 2 · Ce qui change, niveau par niveau

Question : sur quels éléments d'un objet les évolutions portent-elles réellement ?

Je compare l'ENIAC et un ordinateur portable actuel. Pour chaque ligne, j'écris si l'élément a changé, et comment.

| Niveau de l'objet               | ENIAC (1946) | Ordinateur portable actuel |
|---------------------------------|--------------|----------------------------|
| Fonction d'usage                |              |                            |
| Fonction technique « calculer » |              |                            |
| Principe technique du calcul    |              |                            |
| Matériaux                       |              |                            |
| Source d'énergie                |              |                            |
| Design et ergonomie             |              |                            |

e\. Sur les six lignes, combien ont changé ? ......... Laquelle n'a pas changé ? .........

### Activité 3 · Une évolution ne change presque jamais tout

Je lis l'affirmation, je coche, je justifie en une phrase.

| Affirmation                                                                                | Vrai | Faux | Je justifie |
|--------------------------------------------------------------------------------------------|------|------|-------------|
| Quand on fabrique un nouvel objet, on change absolument tout dans sa structure.            |      |      |             |
| Le design des objets n'évolue jamais, seul le service rendu compte.                        |      |      |             |
| Le développement de matériaux nouveaux n'a pas d'influence sur l'évolution des objets.     |      |      |             |
| Faire évoluer un objet petit à petit rend l'objet plus fiable et rassure les utilisateurs. |      |      |             |

## APRÈS — je fixe ce que j'ai appris

#### À retenir

Changer entièrement un objet est rare. Le plus souvent, on fait évoluer quelques **fonctions techniques** : on remplace un composant, on adopte un autre **principe technique**, on ajoute une fonctionnalité.

Procéder par petites évolutions assure la **fiabilité** de l'objet et ne déroute pas les personnes qui l'utilisent déjà.

Le **design** est l'aspect esthétique appliqué à la recherche de formes nouvelles, adaptées à la fonction. Il évolue lui aussi, en général progressivement.

Je complète : entre l'ENIAC et un ordinateur portable, la ......... n'a pas changé, mais le ......... et les matériaux ont changé complètement.

#### Je reviens sur mes hypothèses

Je remonte en haut de la fiche et je coche ✔ ou ✘. J'écris ici ce qui m'a le plus surpris :

#### La question de la prochaine séance

Un ordinateur portable consomme mille fois moins qu'un ENIAC, mais il y en a des milliards. Une évolution technique apporte-t-elle uniquement des avantages ?
