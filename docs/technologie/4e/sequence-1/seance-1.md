# Séance 1 — Des usages qui changent, des objets qui suivent

!!! info "Séance 1 sur 3 · 55 minutes"

    Mettre en relation un OST et ses usages.

    [Fiche élève à imprimer (PDF)](fiches/4e-seq1-seance1-eleve.pdf)

## AVANT — j'entre dans la question

#### Ce qu'on cherche

Le vélo existe depuis près de deux siècles. Il n'a pourtant jamais cessé de changer : cadre, freins, vitesses, assistance électrique, pneus larges pour la neige et le sable.

**Un objet technique change-t-il parce que la technique le permet, ou parce que l'usage l'exige ?**

J'écris trois choses que je crois savoir. Je n'ai pas besoin d'avoir raison : je reviendrai sur ces lignes à la fin de l'heure.

| N°  | Mes hypothèses de départ | Vérifié en fin de séance (✔ / ✘) |
|-----|--------------------------|----------------------------------|
| 1   |                          |                                  |
| 2   |                          |                                  |
| 3   |                          |                                  |

#### Vocabulaire à repérer

| Mot                   | Ce que j'en comprends, avec mes mots |
|-----------------------|--------------------------------------|
| **OST**               |                                      |
| **usage**             |                                      |
| **besoin qui évolue** |                                      |

## PENDANT — j'investigue

**OST**Objet ou système technique. Le programme utilise ce sigle pour désigner aussi bien un objet isolé (une perceuse) qu'un ensemble organisé (un réseau de tramway).

### Activité 1 · Quatre situations, quatre évolutions

Question : pour chaque objet, quelle évolution du besoin a provoqué l'évolution de l'objet ?

Je complète le tableau. La colonne du milieu est la plus importante : elle doit décrire un changement dans la vie des utilisateurs, pas un progrès technique.

<table>
<colgroup>
<col style="width: 33%" />
<col style="width: 33%" />
<col style="width: 33%" />
</colgroup>
<thead>
<tr class="header">
<th style="width: 40mm">L'objet a évolué ainsi</th>
<th>Quel changement dans la vie des gens l'explique ?</th>
<th style="width: 34mm">Quel nouvel usage devient possible ?</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td>→<br />
Le téléphone fixe est devenu un téléphone portable, puis un téléphone connecté.</td>
<td class="vide"></td>
<td></td>
</tr>
<tr class="even">
<td>→<br />
Le vélo classique est devenu un vélo à assistance électrique.</td>
<td class="vide"></td>
<td></td>
</tr>
<tr class="odd">
<td>→<br />
L'appareil photo argentique est devenu numérique, puis s'est fondu dans le téléphone.</td>
<td class="vide"></td>
<td></td>
</tr>
<tr class="even">
<td>→<br />
Le VTT est devenu un fatbike à pneus ultralarges.</td>
<td class="vide"></td>
<td></td>
</tr>
</tbody>
</table>

### Activité 2 · Le même objet, deux époques

Question : que révèle la comparaison d'un objet avec son ancêtre direct ?

J'observe les deux réfrigérateurs ci-dessous, puis je complète.

\*Réfrigérateur des années 1970\*\*Réfrigérateur actuel\*

| Ce que je compare                      | Années 1970 | Aujourd'hui |
|----------------------------------------|-------------|-------------|
| Fonction d'usage                       |             |             |
| Fonctions techniques assurées          |             |             |
| Consommation d'énergie                 |             |             |
| Design (forme, matériaux, couleur)     |             |             |
| Informations affichées à l'utilisateur |             |             |

a\. La fonction d'usage a-t-elle changé ? ......... . Qu'est-ce qui a changé, alors ?

b\. Je formule en une phrase le besoin nouveau auquel répond le réfrigérateur actuel et auquel celui des années 1970 ne répondait pas.

### Activité 3 · Justifier une évolution

Je choisis un OST de la liste : la trottinette, la montre, la voiture, le tableau de la salle de classe, le cartable. J'écris quatre phrases construites ainsi :

« L'objet *…* a évolué parce que le besoin de *…* a changé : autrefois *…*, aujourd'hui *…*. Cette évolution a permis de *…*. »

## APRÈS — je fixe ce que j'ai appris

#### À retenir

Un **OST** (objet ou système technique) est toujours lié à un **usage** : ce que les gens en font réellement, dans un lieu et à une époque donnés.

Les objets techniques évoluent pour deux grandes raisons : répondre à de nouveaux besoins, ou profiter de nouvelles possibilités techniques. Le plus souvent, les deux agissent ensemble.

Quand l'usage change, l'objet change ; quand l'objet change, il rend possibles des usages que personne n'avait prévus.

Je complète : le réfrigérateur d'aujourd'hui rend le ......... service qu'en 1970, mais il répond en plus au besoin de ......... .

#### Je reviens sur mes hypothèses

Je remonte en haut de la fiche et je coche ✔ ou ✘. J'écris ici ce qui m'a le plus surpris :

#### La question de la prochaine séance

Si l'objet change, tout change-t-il en lui d'un seul coup ? Qu'est-ce qui reste, au juste, d'un objet à celui qui le remplace ?
