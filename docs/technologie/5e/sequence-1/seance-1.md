# Séance 1 — De quoi avons-nous besoin ?

!!! info "Séance 1 sur 3 · 55 minutes"

    Besoin, objet technique, fonction d'usage.

    [Fiche élève à imprimer (PDF)](fiches/5e-seq1-seance1-eleve.pdf)

    [Trace écrite de la séance](trace-ecrite-seance-1.md)

## AVANT — j'entre dans la question

#### Ce qu'on cherche

Une journée ordinaire : je me lève, je mange, je viens au collège, j'écoute de la musique, je rentre chez moi. Combien d'objets ai-je utilisés sans y penser ?

**Pour vivre, l'être humain a-t-il simplement besoin d'eau et de nourriture ?**

J'écris trois choses que je crois savoir. Je n'ai pas besoin d'avoir raison : je reviendrai sur ces lignes à la fin de l'heure.

| N°  | Mes hypothèses de départ | Vérifié en fin de séance (✔ / ✘) |
|-----|--------------------------|----------------------------------|
| 1   |                          |                                  |
| 2   |                          |                                  |
| 3   |                          |                                  |

#### Vocabulaire à repérer

| Mot                  | Ce que j'en comprends, avec mes mots |
|----------------------|--------------------------------------|
| **besoin**           |                                      |
| **objet technique**  |                                      |
| **fonction d'usage** |                                      |

## PENDANT — je recherche

### Activité 1 · Ma journée en objets

Question : de quoi ai-je eu besoin depuis mon réveil, et qu'ai-je utilisé pour y répondre ?

Je complète le tableau avec au moins six moments de ma journée d'hier. Je remplis d'abord les deux premières colonnes, puis la troisième.

| Moment de la journée | Ce dont j'avais besoin | L'objet que j'ai utilisé |
|----------------------|------------------------|--------------------------|
| 7 h 00               |                        |                          |
|                      |                        |                          |
|                      |                        |                          |
|                      |                        |                          |
|                      |                        |                          |
|                      |                        |                          |

### Activité 2 · Mettre de l'ordre dans mes besoins

Question : tous ces besoins se ressemblent-ils ?

Avec mon voisin, je range les besoins trouvés à l'activité 1 dans les cases ci-dessous. Un besoin peut n'aller nulle part : j'ajoute alors une case.

| Se nourrir, boire       | Se déplacer  | Communiquer |
|-------------------------|--------------|-------------|
|                         |              |             |
| Se protéger, se soigner | Se distraire | S'instruire |
|                         |              |             |

a\. Les besoins d'un élève de 5<sup>e</sup> à Tegucigalpa sont-ils les mêmes que ceux d'un élève de 5<sup>e</sup> dans un village de montagne ? Je donne un exemple de différence.

b\. Mes besoins d'aujourd'hui seront-ils les mêmes à 50 ans ? Je donne un exemple.

### Activité 3 · À chaque objet son service

Question : à quoi sert vraiment un objet technique ?

La **fonction d'usage** répond à la question « à quoi ça sert ? ». Je l'écris toujours par un verbe à l'infinitif : *permettre de*…

| Objet technique       | Fonction d'usage (verbe à l'infinitif) | Besoin satisfait |
|-----------------------|----------------------------------------|------------------|
| Une perceuse          |                                        |                  |
| Un stéthoscope        |                                        |                  |
| Un pont               |                                        |                  |
| Un extincteur         |                                        |                  |
| Un casque de chantier |                                        |                  |
| Une tondeuse          |                                        |                  |

d\. Le robot NAO peut se déplacer, vous reconnaître, vous entendre et même vous parler. D'après ce texte, à quel besoin répond-il ? Je coche une seule case.

□ Amuser les enfants    □ Tenir compagnie    □ Décorer la maison

e\. Le besoin satisfait par un clavier d'ordinateur est :

□ a. taper sur des touches de manière distincte et facile.  
□ b. permettre à l'utilisateur de transmettre des informations à l'ordinateur.

## APRÈS — je fixe ce que j'ai appris

#### À retenir

!!! note "Deux documents, deux usages"

    Ce bloc se complète en classe, à la fin de l'heure : les phrases à trous se remplissent pendant
    la mise en commun. La [trace écrite de la séance](trace-ecrite-seance-1.md) reprend les mêmes
    notions rédigées et corrigées, avec les définitions exactes, les exemples datés et les
    compétences évaluées. C'est elle qui se colle dans le cahier.

Un **besoin** est un manque que l'être humain cherche à satisfaire. Nos besoins sont nombreux : certains viennent de notre corps, d'autres de la vie en groupe, d'autres du confort.

Ils changent selon le lieu où l'on vit, selon l'âge, et même selon le moment de la journée.

Pour satisfaire un besoin, on utilise un **objet technique** : un objet fabriqué par l'être humain. Le service qu'il rend s'appelle sa **fonction d'usage**.

Je complète : un objet technique est créé pour ......... .

La fonction d'usage de la trottinette est de ......... .

#### Je reviens sur mes hypothèses

Je remonte en haut de la fiche et je coche ✔ ou ✘. J'écris ici ce qui m'a le plus surpris :

#### La question de la prochaine séance

Pour aller au collège, je peux prendre le bus, le vélo ou la trottinette. Trois objets très différents pour un seul service : comment est-ce possible ?

## S'évaluer

[Ouvrir l'évaluation de la séance :material-arrow-right:](evaluation-seance-1.html){ .md-button .md-button--primary target=_blank }

Seize questions reprenant les exercices de la séquence et les définitions de la fiche. J'indique
mon nom, mon prénom et ma classe : la note s'affiche à la fin et elle est envoyée au
professeur. Je relis la trace écrite avant de commencer.
