# Séance 3 — La lignée d'un objet

!!! info "Séance 3 sur 3 · 55 minutes"

    Collecter, trier et analyser des données ; construire une frise.

    [Fiche élève à imprimer (PDF)](fiches/5e-seq1-seance3-eleve.pdf)

## AVANT — j'entre dans la question

#### Ce qu'on cherche

Mes grands-parents écoutaient de la musique sur un appareil qui pesait un kilo et tenait une seule cassette. J'écoute la même musique sur un téléphone qui tient dans la poche.

**Entre les deux, que s'est-il passé, et dans quel ordre ?**

J'écris trois choses que je crois savoir. Je n'ai pas besoin d'avoir raison : je reviendrai sur ces lignes à la fin de l'heure.

| N°  | Mes hypothèses de départ | Vérifié en fin de séance (✔ / ✘) |
|-----|--------------------------|----------------------------------|
| 1   |                          |                                  |
| 2   |                          |                                  |
| 3   |                          |                                  |

#### Vocabulaire à repérer

| Mot                     | Ce que j'en comprends, avec mes mots |
|-------------------------|--------------------------------------|
| **lignée d'objets**     |                                      |
| **performance**         |                                      |
| **frise chronologique** |                                      |

## PENDANT — je recherche

### Activité 1 · Remettre la lignée dans l'ordre

Question : dans quel ordre ces objets sont-ils apparus, et qu'est-ce qui change à chaque étape ?

Je numérote les objets du plus ancien (1) au plus récent (5), puis je complète les colonnes. Les dates sont dans le tableau, je ne les invente pas.

| N°  | Date               | Objet                         | Sur quoi la musique est-elle enregistrée ? | Combien de morceaux environ ? |
|-----|--------------------|-------------------------------|--------------------------------------------|-------------------------------|
|     | 2001               | Lecteur MP3 Apple             |                                            |                               |
|     | 1979               | Walkman Sony (cassette)       |                                            |                               |
|     | 2010 à aujourd'hui | Téléphone connecté            |                                            |                               |
|     | 1984               | Lecteur CD Sony               |                                            |                               |
|     | 1998               | Lecteur numérique RIO PMP 300 |                                            |                               |

a\. Ces cinq objets rendent-ils le même service ? ......... . Ils forment donc une ......... d'objets.

b\. Une **performance** est un résultat que l'on peut mesurer. Je cite deux performances qui ont augmenté le long de cette lignée : ......... .

### Activité 2 · Construire la frise

Question : comment montrer d'un seul coup d'œil quarante ans d'évolution ?

**Méthode.** 1. Choisir un début et une fin. 2. Choisir une échelle (exemple : 5 ans = 1 cm). 3. Placer les dates dans l'ordre. 4. Donner un titre. 5. Illustrer et aérer.

c\. Ma frise ira de ......... à ......... , soit ......... années. Avec une échelle de 5 ans = 1 cm, elle mesurera ......... cm.

d\. Je trace la frise ci-dessous et j'y place les cinq objets. J'écris le titre au-dessus.

### Activité 3 · Analyser : ce qui change, ce qui reste

Je relis ma frise et je réponds.

e\. Qu'est-ce qui n'a jamais changé entre 1979 et aujourd'hui ?

f\. Qu'est-ce qui a le plus changé ?

g\. Le téléphone connecté sert-il uniquement à écouter de la musique ? Que peut-on en conclure sur la place de cet objet dans la lignée ?

### Exercices en ligne · Je m'entraîne

Les dix questions du QCM de la séquence 1 sont disponibles sur le site de la classe, rubrique **5<sup>e</sup> › Séquence 1 › Exercices**. Le score s'affiche à la fin, l'exercice peut être refait autant de fois que nécessaire.

## APRÈS — je fixe ce que j'ai appris

#### À retenir

Une **lignée d'objets** est la suite chronologique des évolutions d'un objet, du plus ancien au plus récent. Tous les objets d'une lignée rendent le même service.

Une **performance** est un résultat mesurable obtenu par un objet technique : une autonomie en heures, une capacité en nombre de morceaux, une masse en grammes.

Une **frise chronologique** sert à faire comprendre une succession d'événements : elle a un titre, une échelle, et elle reste lisible et aérée.

Je complète : dans une lignée, les objets sont classés par ......... , et leurs performances ......... .

#### Je reviens sur mes hypothèses

Je remonte en haut de la fiche et je coche ✔ ou ✘. J'écris ici ce qui m'a le plus surpris :

#### La question de la prochaine séance

Le lecteur MP3 a presque disparu, avalé par le téléphone. Un objet peut-il donc mourir ? Et qui décide de ce qui remplace quoi ? La séquence 2 ouvrira cette question.

## S'évaluer

[Ouvrir l'évaluation de la séance :material-arrow-right:](evaluation-seance-3.html){ .md-button .md-button--primary target=_blank }

Seize questions reprenant les exercices de la séquence et les définitions de la fiche. J'indique
mon nom, mon prénom et ma classe : la note s'affiche à la fin et elle est envoyée au professeur.
